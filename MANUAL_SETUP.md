...
- [ ] 봇이 세 채널에 초대되어 있는지 확인:
      ...
      병원 `C097BQWHQGG`(선병원) / `C096L5YHEJ2`(한국병원)
...
# 2. 대전한국병원 (port 10003)
cd 병원AI에이전트 && HOSPITAL_PROFILE=configs/대전한국병원.env UV_PROJECT_ENVIRONMENT=~/.venvs/ambulance-병원AI에이전트 uv run python __main__.py
