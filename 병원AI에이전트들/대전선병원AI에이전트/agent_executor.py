...
class HospitalAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs Hospital AI Agent of 대전선병원."""
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id="대전선병원AI에이전트", new_message=new_message
        )
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="대전선병원AI에이전트", session_id=session_id
        )
        if session is None:
